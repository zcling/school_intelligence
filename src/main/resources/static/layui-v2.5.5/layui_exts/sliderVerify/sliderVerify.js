layui.define(['jquery', 'layer', 'form'], function (exports) {
    "use strict";
    var $ = layui.jquery, form = layui.form, layer = layui.layer, device = layui.device(), sliderVerify = {
            read: function () {
        ...
        }, MOD_NAME = 'sliderVerify', MOD_BTN = 'slider-btn', MOD_BG = 'slider-bg', MOD_TEXT = 'slider-text',
        MOD_NEXT = 'layui-icon-next', MOD_OK = 'layui-icon-ok-circle', MOD_BTN_SUCCESS = 'slider-btn-success',
        MOD_DEFAULT_BG = 'layui-bg-green', MOD_ERROR_BORDER = 'slider-error', MOD_CONFIG_TEXT = '请拖动滑块验证',
        MOD_CONFIG_SUCCESS = '验证通过', Class = function (option) {
            var that = this;
    ...
    Class.prototype.render = function () {
        var that = this, option = that.config, elem = $(option.elem);
        if (!elem[0]) return;
        if (option.domid) option.domid.remove();
        option.domid = that.createIdNum();
        var sliderDom = $(`<div id="${option.domid}"${option.isAutoVerify ? 'lay-verify="sliderVerify"' : ''}class="slider-item"><div class="${MOD_BG} ${option.bg}"></div><div class="${MOD_TEXT}">${option.text}</div><div class="${MOD_BTN} layui-icon layui-icon-next"></div></div>`);
    ...
    Class.prototype.distance = function () {
        ...
        return container.box.offsetWidth - container.btn.offsetWidth
    };
    ...
    Class.prototype.resize = function (distance) {
        var that = this, container = that.config.container;
    ...
    Class.prototype.down = function (e) {
        var that = this, option = that.config, container = option.container, e = e || window.event,
    ...
    Class.prototype.events = function () {
        ...
        var btn = option.domid.find('.' + MOD_BTN), bg = option.domid.find('.' + MOD_BG),
            text = option.domid.find('.' + MOD_TEXT),
            container = {box: dom(option.domid), btn: dom(btn), bg: dom(bg), text: dom(text)};
